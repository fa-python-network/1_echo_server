...
logger = logging.getLogger("serverLogger") #получаем новый логгер
logger.setLevel(logging.INFO) #ставит уровень инфо(подтверждение, информация о событиях, не приводящих к ошибкам в работе модулей)
fh = logging.FileHandler("server.log") #отображает лог в файле
formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s') #форматируем по типу
fh.setFormatter(formatter)#устанавлваем формат
logger.addHandler(fh)#регистрируем событие

logger.info("Start server") #Происходит логгирование с фразой Start server
